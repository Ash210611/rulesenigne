...
# ===============================================================================
def create_table_structure(table_name_orig, hive_storage_type):
    G.TABLE_STRUCTURE = C.HiveTableStructure(
        G.DATABASE_BASE,
        table_name_orig,
        G.SQL_STATEMENT_OBJ)
    ...
    G.TABLE_STRUCTURE.hive_storage_type = hive_storage_type

    G.HIVE_TABLE_STRUCTURES.append(G.TABLE_STRUCTURE)
    G.TABLE_STRUCTURES = G.HIVE_TABLE_STRUCTURES
...
def read_create_table():
    ...
    get_table_comment = False
    found_table_comment = False
    table_comment_str = ''
    hive_storage_type = ''
    G.TABLE_STRUCTURE = None

    for line in G.ANTLR_LOG_CONTENTS.split('\n'):
        if get_table_comment:
            get_table_comment = False
            if re.search(r'Found comment-on string      : ', line, re.IGNORECASE):
                found_table_comment = True
                table_comment_str = split_value_from_line(line)
                table_comment_str = table_comment_str.strip('"')
                table_comment_str = table_comment_str.strip("'")
            # The next line in the Antlr file should show the table name

        elif re.search(r'Found comment-on object      : ', line, re.IGNORECASE):
            object_type = split_value_from_line(line.upper())
            if object_type == 'TABLE':
                get_table_comment = True

        elif re.search(r'Found table storage as       : ', line, re.IGNORECASE):
            hive_storage_type = split_value_from_line(line)
        ...
        if re.search(r'Found database identifier    : ', line, re.IGNORECASE):
            ...
            G.TABLE_STRUCTURE = create_table_structure(G.TABLE_NAME, hive_storage_type)

    if found_table_comment:
        G.TABLE_COMMENT = C.TableComment(
            G.DATABASE_BASE,
            G.TABLE_NAME,
            table_comment_str,
...
# ===============================================================================
def add_column_element(name, datatype, title, comment_txt, is_identity):
    '''
    ...
    # The languages for these rules engine types create comments
    # in the Create Table command
    if comment_txt is not None:
        comment_txt = comment_txt.strip("'")
        comment_txt = comment_txt.strip('"')

        column_comment = C.ColumnComment(G.DATABASE_BASE,
                                         G.TABLE_STRUCTURE.table_name_upper,
                                         name,
                                         comment_txt,
                                         G.INPUT_FILENAME,
...
def read_column_elements():
    ...
    for line in G.ANTLR_LOG_CONTENTS.split('\n'):
        ...
        if re.search(r'Found comment-on string      :', line):
            # Hive includes comments in the Create Table command
            comment_txt = split_value_from_line(line)

        elif re.search(r'Found column_name            :', line):
            if column_should_be_added and not column_is_added:
                add_column_element(name, datatype, title, comment_txt, is_identity)
                datatype = None
                comment_txt = None
                title = ''
                is_identity = False
...
# ===============================================================================
def get_hive_sql_statement_obj(antlr_log_filename):
    '''
...
# ===============================================================================
def get_hive_antlr_findings(antlr_log_filename):
    ...
    G.SQL_STATEMENT_OBJ = get_hive_sql_statement_obj(antlr_log_filename)
